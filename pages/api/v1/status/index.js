...
async function status(request, response) {
  ...
  const dataabaseOpenConnectionsResults = await database.query({
    text: "SELECT count(*)::int FROM pg_stat_activity WHERE datname = $1;",
    values: [databaseName]
  });
  const connectionsBeingUsedValueValue = dataabaseOpenConnectionsResults.rows[0].count
  ...
  response.status(200).json({
    ...
    dependencies: {
      database: {
        ...
        connections_being_used: connectionsBeingUsedValueValue,
      },
    },
  });
}
